...
nDots = sum([dots.nDots]); % same as cat(1,dots.nDots)
...
for i=1:length(dots) %Loop through the fields
    ...
    direction = rand(1,dots(i).nDots)*360; %Start w/ all random directions
    nCoherent = ceil(dots(i).coherence*dots(i).nDots);  % find the proportion of dots moving in the main direction 
