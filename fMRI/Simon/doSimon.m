...
function  s=doSimon(display,s,curTime,keybs)
%s=doSimon(display,s,curTime)
% modified to avoid using blue (blue button not working properly) 9/23/11 pb
% blue put back in 3/17/13 pb
if ~exist('s','var')
    s.action = 'init';
end
...
switch s.action
    case 'init'  %set up simon variables
        % 'simon' parameters
        s.size = [6,20,24,3];  %pixels
        s.gap = 2; %pixels
        s.c = ceil(display.resolution/2);
