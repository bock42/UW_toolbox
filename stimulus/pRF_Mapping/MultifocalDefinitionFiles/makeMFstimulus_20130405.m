...
if ~createMFstim
    ...
    % ADAPT TO TR=1s (but blocks still last 6s)
    
    % because each event is repeated twice, sample every second TR
    stimSeq = stimSeq(:,1:2:end);
    % now repeat each event 6 times (so that 
    
    tmp = []; 
    for i = 1:size(stimSeq,2)
        tmp = [tmp, repmat(stimSeq(:,i),1,nreps)];
    end
    
    stimSeq = tmp;
    fprintf('n of TRs: %i, n of sec: %i \n',length(stimSeq),length(stimSeq)*myTR);
...
%     plot(time_old, stimSeq_bu(1,:),'b.')
%     plot(time_new, stimSeq(1,:),'ro')
%     ylim([-1 2])
...
if createMFstim 
    ...
    save MFStimLayoutAndSequence.mat regDef stimSeq myTR
end
